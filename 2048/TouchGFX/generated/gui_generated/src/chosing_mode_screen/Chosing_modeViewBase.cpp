/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#include <gui_generated/chosing_mode_screen/Chosing_modeViewBase.hpp>
#include <touchgfx/Color.hpp>
#include <texts/TextKeysAndLanguages.hpp>

Chosing_modeViewBase::Chosing_modeViewBase() :
    flexButtonCallback(this, &Chosing_modeViewBase::flexButtonCallbackHandler)
{
    __background.setPosition(0, 0, 240, 320);
    __background.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    add(__background);

    box1.setPosition(0, 0, 240, 320);
    box1.setColor(touchgfx::Color::getColorFromRGB(115, 88, 88));
    add(box1);

    flexButton1.setBoxWithBorderPosition(0, 0, 131, 54);
    flexButton1.setBorderSize(5);
    flexButton1.setBoxWithBorderColors(touchgfx::Color::getColorFromRGB(0, 102, 153), touchgfx::Color::getColorFromRGB(0, 153, 204), touchgfx::Color::getColorFromRGB(0, 51, 102), touchgfx::Color::getColorFromRGB(51, 102, 153));
    flexButton1.setAction(flexButtonCallback);
    flexButton1.setPosition(55, 75, 131, 54);
    add(flexButton1);

    flexButton2.setBoxWithBorderPosition(0, 0, 131, 50);
    flexButton2.setBorderSize(5);
    flexButton2.setBoxWithBorderColors(touchgfx::Color::getColorFromRGB(0, 102, 153), touchgfx::Color::getColorFromRGB(0, 153, 204), touchgfx::Color::getColorFromRGB(0, 51, 102), touchgfx::Color::getColorFromRGB(51, 102, 153));
    flexButton2.setAction(flexButtonCallback);
    flexButton2.setPosition(56, 145, 131, 50);
    add(flexButton2);

    textArea1_1.setXY(91, 93);
    textArea1_1.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    textArea1_1.setLinespacing(0);
    textArea1_1.setTypedText(touchgfx::TypedText(T___SINGLEUSE_NQUE));
    add(textArea1_1);

    textArea1.setXY(63, 161);
    textArea1.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    textArea1.setLinespacing(0);
    textArea1.setTypedText(touchgfx::TypedText(T___SINGLEUSE_X5PN));
    add(textArea1);

    flexButton3.setBoxWithBorderPosition(0, 0, 131, 50);
    flexButton3.setBorderSize(5);
    flexButton3.setBoxWithBorderColors(touchgfx::Color::getColorFromRGB(0, 102, 153), touchgfx::Color::getColorFromRGB(0, 153, 204), touchgfx::Color::getColorFromRGB(0, 51, 102), touchgfx::Color::getColorFromRGB(51, 102, 153));
    flexButton3.setAction(flexButtonCallback);
    flexButton3.setPosition(56, 210, 131, 50);
    add(flexButton3);

    textArea4.setXY(96, 226);
    textArea4.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    textArea4.setLinespacing(0);
    textArea4.setTypedText(touchgfx::TypedText(T___SINGLEUSE_OLMF));
    add(textArea4);

    textArea5.setXY(55, 27);
    textArea5.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    textArea5.setLinespacing(0);
    textArea5.setTypedText(touchgfx::TypedText(T___SINGLEUSE_4PZ6));
    add(textArea5);
}

Chosing_modeViewBase::~Chosing_modeViewBase()
{

}

void Chosing_modeViewBase::setupScreen()
{

}

void Chosing_modeViewBase::flexButtonCallbackHandler(const touchgfx::AbstractButtonContainer& src)
{
    if (&src == &flexButton1)
    {
        //Interaction1
        //When flexButton1 clicked change screen to SelectedGameDesign
        //Go to SelectedGameDesign with screen transition towards East
        application().gotoSelectedGameDesignScreenSlideTransitionEast();
    }
    if (&src == &flexButton2)
    {
        //Interaction2
        //When flexButton2 clicked change screen to Screen5x5_superMerging
        //Go to Screen5x5_superMerging with screen transition towards East
        application().gotoScreen5x5_superMergingScreenSlideTransitionEast();
    }
    if (&src == &flexButton3)
    {
        //Interaction3
        //When flexButton3 clicked change screen to Screen5x5_letterMerging
        //Go to Screen5x5_letterMerging with screen transition towards East
        application().gotoScreen5x5_letterMergingScreenSlideTransitionEast();
    }
}
